...
- **Done:** commit remaining changes and push the branch, then follow the resolved **finalize mode**. `:pull_request` — if the remote is GitHub or GitLab, create an MR/PR via API (token from the project env store) or show a compare URL; the remote branch stays. The MR/PR body is rendered from the project's editable `pr_template` (`{{title}}`/`{{description}}`/`{{task_id}}`/`{{branch}}` placeholders), defaulting to a built-in template when the project has not set one; the title is always `tasks.title`. `:merge` / `:squash` — merge locally (below) and delete the remote feature branch.
