...
once any user exists. The admin form only asks for a **username** and
password; email is optional, since most self-hosted instances have no
working mail adapter at all (see "Username, not email" below).
...
1.8, scope-based, magic link + password, bcrypt), then adapted so a
working instance never needs email at all. Deviations from the
generated output:

- **Username, not email, is the login identifier.** `users.username`
  (added by `20260816120942_add_username_to_users.exs`) is required and
  unique; `users.email` is now nullable and only matters for the
  magic-link/invite flow below. The login page's password form asks for
  a username; the DB migration backfills a `username` for pre-existing
  rows from the local-part of their email (de-duplicated), since this
  ships to instances that already have users. `User.changeset/3` folds
  both fields together — `validate_unique: false` skips the live-query
  uniqueness checks for both, the same switch `email_changeset/3` already
  had.
