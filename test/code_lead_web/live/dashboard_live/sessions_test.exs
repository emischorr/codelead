defmodule CodeLeadWeb.DashboardLive.SessionsTest do
  ...
  describe "counting" do
    ...
    test "the detail names two tasks then tallies the rest", %{conn: conn} do
      project = project_fixture()
      ...
      detail = tile(view, "tile-previews")
      assert tile_value(view, "tile-previews") == "3"
      assert detail =~ "##{first.id} Alpha"
      assert detail =~ "##{second.id} Beta"
      assert detail =~ "+1 more"
    end
  end
